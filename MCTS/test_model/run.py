...
ph_x = tf.placeholder(tf.float32, shape=[None])
ph_y = tf.placeholder(tf.float32, shape=[None])
x_with_bias = tf.stack([ph_x, tf.ones_like(ph_x)], axis=1)
pred = tf.reshape(tf.matmul(x_with_bias, wb), [-1])
loss = tf.reduce_mean((ph_y-pred)**2)
