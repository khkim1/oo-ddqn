...
class Detector(object):
    ...
    def draw_result(self, img, result, obj=None):
    ...
    def detect(self, img):
        life_bar = self.get_lifebar(img)
        inputs = self.cut_scale(img)

        #np.save('frame', inputs)
        #print(np.shape(inputs))
        #plt.imshow(inputs[0, :, :])
        #plt.show()
        results = self.detect_from_cvmat(inputs)[0]

        net_output = np.zeros((20, 11))

        if results:
            #inputs = inputs[0, :, :]
            #self.draw_result(inputs, results)
            #plt.imshow(inputs)
            #plt.show()
            ...
            num_objects, _ = np.shape(results)

            #print(results)
            one_hots = keras.utils.to_categorical(results[:, 0], num_classes=6)
            if one_hots.ndim == 1:
                one_hots = one_hots.reshape((1, 6))
            rep = np.concatenate((one_hots, results[:, 1:-1]/41.5 - 1., np.expand_dims(results[:, -1], 1)), axis=1)
            net_output[0:num_objects, :] = rep

        net_output = net_output.reshape((1, 220, 1))
        return np.concatenate((net_output,
                               life_bar), axis=1)
