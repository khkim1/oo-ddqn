...
while True:
  ...
  '''
  for idx in range(3):
    cv2.circle(I, (int(cur_x[2 * idx] * scale_down), int(cur_x[2 * idx + 1] * scale_down)),
               1, (255, 0, 0), -1)
  cv2.imshow('Image', I)
  cv2.waitKey(1)
  '''
  ...
  '''
  if not np.array_equal(next_x, np.zeros(6)) and not np.array_equal(cur_x, np.zeros(6)):
    cur_dist = np.linalg.norm(cur_x[-2:] - cur_x[-4:-2], 2)
    next_dist = np.linalg.norm(next_x[-2:] - next_x[-4:-2], 2)

    if next_dist < 0.5 and next_x[-3] - cur_x[-3] > 0:
      bonus = cur_dist - next_dist
      reward += bonus / 100
  '''
